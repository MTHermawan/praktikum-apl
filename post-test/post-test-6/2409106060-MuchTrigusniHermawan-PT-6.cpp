...
void UpdateJumlahMod(Game *gamesPtr, int &gameCount, Mod *modsPtr, int &modCount)
{
    for (int i = 0; i < gameCount; i++)
    {
        ...
        {
            if (gamesPtr[i].nama == modsPtr[j].game->nama)
                jumlahModBaru++;
        }
        gamesPtr[i].jumlahMod = jumlahModBaru;
    }
}
...
void MenuGame(char &halaman, string &pilihan_menu, Game &selectedGame, string selectedGenres[], int &selectedGenreCount, Game *gamesPtr, int &gameCount, Mod *modsPtr, int &modCount, GabunganArray *dataArrayPtr, int &genreCount, User *currentUser, int &gameSortType)
{
    string _temp;
    int i;
    ...
    {
        cout << left << setw(6) << "No."
             << setw(40) << (gameSortType == 1 ? "Game (Asc)" : "")
             << setw(10) << "Jumlah Mod " << (gameSortType == 2 ? "(Desc)" : "")
             << endl;
        Border("─", 106);
...
void MenuMod(char &halaman, string &pilihan_menu, Game selectedGame, Mod &selectedMod, Game *gamesPtr, int &gameCount, Mod *modsPtr, int &modCount, User *currentUser, int &modSortType)
{
    string _temp = "";
    ...
    if (modListMenuCount > 0)
    {
        // Daftar Mod
        cout << left << setw(6) << "No."
             << setw(40) << (modSortType == 1 ? "Nama Mod (Asc)" : modSortType == 2 ? "Nama Mod (Desc)" : "")
             << setw(40) << "Game"
             << setw(20) << "Creator"
             << endl;
        Border("─", 106);
...
int main()
{
    ...
    Game games[MAX_DATA] = {
        {"Stardew Valley", {"Simulation", "RPG", "Indie"}, 1, 3},
        {"Undertale", {"RPG", "Indie"}, 1, 2},
        {"Terraria", {"Action", "RPG"}, 2, 2},
        {"Fallout 4", {"RPG"}, 0, 1},
        {"Skyrim", {"RPG"}, 0, 1}};
    int gameCount = 5;
    ...
    Mod mods[MAX_DATA] = {
        ...
        {4, &games[2], "Thorium Mod", "DivermanSam", &users[0], "The Thorium Mod is a \"Vanilla+\" content pack that adds a massive variety of new items, weapons, armors, NPCs, bosses, and even 3 new classes into the vast world of Terraria!."}};
    int modCount = 4;
    ...
    {
        UpdateJumlahMod(games, gameCount, mods, modCount);
        ...
        else
        {
            ...
            case '1':
                // Menu Game
                MenuGame(halaman, pilihan_menu, selectedGame, selectedGenres, selectedGenreCount, games, gameCount, mods, modCount, dataArray, genreCount, currentUser, gameSortType);
                break;
            case '2':
                // Menu Mod
                MenuMod(halaman, pilihan_menu, selectedGame, selectedMod, games, gameCount, mods, modCount, currentUser, modSortType);
                break;
            case '3':
                // Filter Game
